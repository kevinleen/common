...
bool LogReader::readRecord(std::string* log, bool* is_last) {
  ...
  uint32 len = v32(data);
  data += 4;
  CHECK_LE(len, load_size_ - offset_);
  CHECK_LE(len, BLOCK_SIZE - offset_);
  ...
  if (crc_check_ && saved_crc != 0) {
    uint32 crc = crc32Value(data, len);
    if (crc != saved_crc) {
      return false;
    }
  }
